...
learning and possible existent color vision deficiencies. Color is also frequently used to convey information, while associating it
...
data variables simultaneously? Over the last years, previous investigation has not come to an agreement
...
is the one which generates shorter distances between the reference colors and the ones indicated. Whilst, the CIE-L*C*h* Model is the
one which is farther apart from the users’ mental model of color. We have also detected that there is a mild indicator that there exists
