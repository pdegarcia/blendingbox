...
laboratory experience, since a Wilcoxon Test show that there are no statistically significant differences between the laboratory distances and online ones ($p < 0.05$).
%
\begin{table}[H]
  \resizebox{\textwidth}{!} {
  \begin{tabular}{lccccccccccccc}
...
%
Evaluating the amount of one-white-component answers \emph{per} color blending, we detect that the ones which had the most were \textbf{Green-Magenta to produce a shade of Blue} (63 answers), \textbf{Red-Green to produce Yellow} (60 answers), and
\textbf{Blue-Yellow to produce a shade of Magenta} (56 Answers).
%
\begin{figure}[!htbp]
  \centering
  \begin{minipage}{0.48\textwidth}
...
which presented the lowest mean value for distances was the CMYK Color Model}. The RGB, HSV and CIE-L*a*b* Color Models all generate values very similar to each other, so \textbf{further studies should focus on discernin which color model is better than others, between
...
on the process of creating Magenta: users have indicated more detailed shades of Red to create the color mixture, contrary to other color blends evaluated, which may have implications on the level of detail that the displaying of color should have. \par
%
...
the rating of two and three, on a scale from one to five, which could indicate that apart from the obvious color blendings which proved to yield the best results, all \textbf{the other blendings show nor an easy or hard level of effort to mix the basis}.
%
...
demonstrate significant comparisons. In this section, we handle a brief analysis, exemplifing with one example \par
%
...
Indicar se existe ou não alguma ordem em particular para indicar cores (Magenta e Yellow, cor primaria)
%
...
CMYK em vez de HSV
Escalas de cor devem incluir branco
Escalas de cor continuas dão mais liberdade ao utilizador para indicar outras cores, mas criam entropia. Escalas de cor discretas são mais incisivas nos valores, mas criam restrição.
Eventualmente, seria interessante explorar a vertente de mudar a interface da visualização consoante utilizador Male ou Female, mas são necessários estudos adicionais.
Indicar se é melhor indicar as cores já misturadas, ou dar espaço ao utilizador para misturar as bases.
%
